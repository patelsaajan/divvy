  ...
  <div class="container mx-auto mt-6">
    ...
    <div class="flex justify-between items-center mb-3 pb-3 border-b border-gray-700">
      ...
      <div class="text-right">
        <span class="font-semibold text-lg">${{ expense.total.toFixed(2) }}</span>
        <!-- <div class="text-xs text-gray-400">{{ receiptItems.data.value.length }} items</div> -->
      </div>
    </div>

    <!-- Receipt Line Items -->
    <div v-show="status === 'success'" class="space-y-2">
      <div 
      ...
      >
        ...
        <div class="font-medium">${{ item.cost.toFixed(2) }}</div>
      </div>
    </div>
...
const { data: receiptItems, status } = useAsyncData('receipt_items', async () => {
