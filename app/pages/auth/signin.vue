...
const signInWithOtp = async () => {
  ...
  if (error) {
    if (error.status === 422) {
      errorMsg.value = 'No account found for this email. You may need to create an account.'
    } else {
  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex items-center justify-center px-4">
    <div class="w-full max-w-md">
      ...
      <div class="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-8 border border-gray-700/50">
        <!-- Error Alert -->
        <div v-if="errorMsg" class="mb-4 p-4 bg-red-500/10 border border-red-500/30 rounded-lg">
          <h3 class="text-red-400 font-medium">Error</h3>
          <p class="text-red-300 text-sm mt-1">{{ errorMsg }}</p>
        </div>

        <!-- Sign In Form -->
        <form v-if="!isEmailSent" @submit.prevent="signInWithOtp" class="space-y-4">
          <div>
